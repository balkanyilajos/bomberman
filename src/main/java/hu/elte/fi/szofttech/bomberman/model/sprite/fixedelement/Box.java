...
public class Box extends Sprite {
    ...
    public Box(GameModel model, Point2D point) {
        this(model, point, model.getCubeSize(),
                new Point2D.Double(point.getX() - model.getCubeSize().width * 2 * 0.3,
                        point.getY() - model.getCubeSize().height * 1.4 * 0.18),
    ...
    public Box(GameModel model, Point2D areaPoint, Dimension areaSize, Point2D imagePoint, Dimension imageSize) {
        super(model,
                new Area(new Rectangle2D.Double(areaPoint.getX(), areaPoint.getY(), areaSize.width, areaSize.height)),
                imagePoint, areaPoint, imageSize, GeneralPath.getPath() + "/picture/box/0.png");
        ...
        for (int i = 1; i <= 8; i++) {
            try {
                elimination.add(ImageIO.read(new File(GeneralPath.getPath() + "/picture/box/" + i + ".png")));
            } catch (IOException e) {
